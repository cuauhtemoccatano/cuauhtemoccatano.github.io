...
## 2025-05-14 - [Focus Visibility with Custom Cursors]
**Learning:** In environments where `cursor: none !important` is used, standard focus indicators are often suppressed or ignored during design. This makes high-contrast `:focus-visible` outlines critical for keyboard users who cannot rely on a visible mouse cursor for orientation.
**Action:** Always implement a global `:focus-visible` rule with `!important` on the outline property to ensure visibility is restored on all interactive elements, even if they have `outline: none` in their base styles.
