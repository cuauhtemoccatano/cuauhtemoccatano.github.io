...
## 2026-03-18 - [Localized Accessibility Attributes]
**Learning:** For a multi-language site, icon-only buttons need ARIA labels that update dynamically. A reusable pattern is to use a custom data attribute (e.g., `data-i18n-label`) that the language-switching logic uses to refresh the `aria-label`.
**Action:** Use `data-i18n-label` on all interactive elements that rely on `aria-label` for context, and ensure the `updateLanguage` function iterates over them to apply translations from the central `translations` object.
