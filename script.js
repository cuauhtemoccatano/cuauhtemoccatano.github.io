...
const translations = {
  EN: {
    ...
    switch_light: "Switch to light mode", switch_dark: "Switch to dark mode",
    close_oracle: "Close Oracle", send_message: "Send Message",
    oracle_toggle: "Toggle Oracle Chat", back_to_info: "Back to Information",
    terminal_input_label: "Terminal Input", lang_switch_label: "Switch Language",
    scan_url_label: "Website URL to Scan", scan_now_label: "Scan Now",
    oracle_input_label: "Message Oracle"
  },
  ES: {
    ...
    switch_light: "Cambiar a modo claro", switch_dark: "Cambiar a modo oscuro",
    close_oracle: "Cerrar Oráculo", send_message: "Enviar Mensaje",
    oracle_toggle: "Alternar Chat del Oráculo", back_to_info: "Volver a Información",
    terminal_input_label: "Entrada de Terminal", lang_switch_label: "Cambiar Idioma",
    scan_url_label: "URL del sitio para escanear", scan_now_label: "Escanear ahora",
...
function applyTheme(theme) {
  ...
  // Update theme toggle accessibility labels
  const themeLabel = translations[currentLang][isDark ? 'switch_light' : 'switch_dark'];
  toggleIcons.forEach((btn) => {
    btn.setAttribute("aria-label", themeLabel);
    btn.setAttribute("title", themeLabel);
    btn.setAttribute("data-i18n-label", isDark ? 'switch_light' : 'switch_dark');
    btn.setAttribute("data-i18n-title", isDark ? 'switch_light' : 'switch_dark');
  });
}
...
function updateLanguage(lang) {
  ...
  // 1.1 Localized ARIA Labels and Titles
