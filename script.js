...
toggleIcons.forEach((toggle) => {
  toggle.addEventListener("click", () => {
    ...
    const isLightModeNext = nextTheme !== "dark";
    const labelKey = isLightModeNext ? "switch_dark" : "switch_light";
    const nextLabel = translations[currentLang][labelKey];

    toggleIcons.forEach((btn) => {
      btn.setAttribute("aria-label", nextLabel);
      btn.setAttribute("title", nextLabel);
      btn.setAttribute("data-i18n-label", labelKey);
...
const translations = {
  ...
  ES: {
    ...
    oracle_toggle: "Habla con el Oráculo",
...
function updateLanguage(lang) {
  ...
  // 1.1 Labels and Titles with data-i18n-label and data-i18n-title
