/**
 * Multi-language Support Logic & Theme Management Logic
 */
const translations = {
  EN: {
    home: "Home", about: "About", services: "Services", skills: "Skills", projects: "Engineering", launchpad_hub: "Launchpad", podcasts: "Podcasts", contact: "Contact",
...
function applyTheme(theme) {
  const labelKey = theme === "dark" ? "switch_light" : "switch_dark";
  const text = translations[currentLang] && translations[currentLang][labelKey] ? translations[currentLang][labelKey] : (theme === "dark" ? "Switch to light mode" : "Switch to dark mode");
...
function updateLanguage(lang) {
  ...
  // Support for placeholder specifically
  ...
  // Ensure state-dependent labels (like theme switch) are fully refreshed based on the new language
  const currentTheme = document.body.classList.contains("dark-mode") ? "dark" : "light";
  const labelKey = currentTheme === "dark" ? "switch_light" : "switch_dark";
  const nextText = t[labelKey] || (currentTheme === "dark" ? "Switch to light mode" : "Switch to dark mode");
  toggleIcons.forEach((btn) => {
    btn.setAttribute("data-i18n-aria-label", labelKey);
    btn.setAttribute("data-i18n-title", labelKey);
